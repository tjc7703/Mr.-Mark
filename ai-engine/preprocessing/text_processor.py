...
class TextProcessor:
    ...
    def __init__(self):
        ...
        korean_stop_words = {
            '이', '그', '저', '것', '수', '등', '때', '곳', '말', '일',
            '때문', '그것', '그런', '이런', '저런', '어떤', '무슨', '어느',
            '아무', '모든', '각', '여러', '다른', '같은', '비슷한', '새로운',
            '옛날', '현재', '미래', '과거', '지금', '이제', '그때', '저때'
        }
    ...
    def _load_sentiment_dict(self) -> Dict[str, float]:
        ...
        return {
            # 긍정적 단어들
            '좋다': 1.0, '훌륭하다': 1.0, '멋지다': 1.0, '완벽하다': 1.0,
            '최고': 1.0, '최고다': 1.0, '사랑': 1.0, '사랑한다': 1.0,
            '감동': 1.0, '감동적': 1.0, '행복': 1.0, '행복하다': 1.0,
            '즐겁다': 1.0, '재미있다': 1.0, '유용하다': 1.0, '도움이': 1.0,
            
            # 부정적 단어들
            '나쁘다': -1.0, '최악': -1.0, '최악이다': -1.0, '싫다': -1.0,
            '짜증': -1.0, '짜증나다': -1.0, '화나다': -1.0, '분노': -1.0,
            '실망': -1.0, '실망하다': -1.0, '슬프다': -1.0, '우울하다': -1.0,
            '힘들다': -1.0, '어렵다': -1.0, '복잡하다': -1.0, '불편하다': -1.0,
            
            # 중립적 단어들
            '보통': 0.0, '일반적': 0.0, '평범하다': 0.0, '그저': 0.0,
            '그냥': 0.0, '그대로': 0.0, '동일하다': 0.0, '같다': 0.0
        }
    ...
    def extract_topics(self, texts: List[str], num_topics: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # TF-IDF 점수순 정렬
            sorted_topics = sorted(all_keywords.items(), key=lambda x: x[1]['tfidf_score'], reverse=True)
...
def main():
    ...
    for result in results:
        ...
        print(f"감정: {result['sentiment']['sentiment_label']} ({result['sentiment']['overall_sentiment']:.2f})")
