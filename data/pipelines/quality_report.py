...
class QualityReporter:
    ...
    async def generate_quality_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 리포트 저장
            report_path = self.reports_path / f"quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def _analyze_table_quality(self, conn: sqlite3.Connection, table: str) -> Dict[str, Any]:
        """테이블별 품질 분석"""
        try:
            # 테이블 존재 여부 확인
            cursor = conn.cursor()
    ...
    async def _calculate_completeness(self, conn: sqlite3.Connection, table: str) -> float:
        """완성도 계산"""
        try:
            cursor = conn.cursor()
    ...
    async def _calculate_consistency(self, conn: sqlite3.Connection, table: str) -> float:
        """일관성 계산"""
        try:
            cursor = conn.cursor()
    ...
    async def _calculate_timeliness(self, conn: sqlite3.Connection, table: str) -> float:
        """최신성 계산"""
        try:
            cursor = conn.cursor()
            ...
            if 'created_at' in columns:
                ...
                if latest_time:
                    ...
                    time_diff = datetime.now().replace(tzinfo=latest_dt.tzinfo) - latest_dt
                    
                    # 5분 이내면 1.0, 그 외에는 비례하여 감소
                    if time_diff.total_seconds() <= 300:  # 5분
                        return 1.0
                    else:
                        return max(0.0, 1.0 - (time_diff.total_seconds() - 300) / 3600)  # 1시간당 0.1 감소
    ...
    async def _calculate_uniqueness(self, conn: sqlite3.Connection, table: str) -> float:
        """고유성 계산"""
        try:
            cursor = conn.cursor()
    ...
    async def _identify_quality_issues(self, conn: sqlite3.Connection, quality_results: Dict[str, Any]) -> List[QualityIssue]:
        """품질 이슈 식별"""
        issues = []
        ...
        for table, result in quality_results.items():
            if isinstance(result, dict) and 'metrics' in result:
                ...
                for metric_name, score in metrics.items():
                    ...
                    if score < threshold:
                        severity = 'critical' if score < threshold * 0.5 else 'high' if score < threshold * 0.8 else 'medium'
    ...
    def _get_recommendation(self, metric_name: str, table: str) -> str:
        ...
        return recommendations.get(metric_name, f"{table} 테이블의 데이터 품질 개선이 필요합니다.")
    ...
    async def _generate_recommendations(self, issues: List[QualityIssue]) -> List[str]:
        ...
        if critical_issues:
            recommendations.append("🚨 긴급 조치 필요: 데이터 품질이 심각한 수준입니다. 즉시 데이터 검증 및 정제 작업을 수행하세요.")
        
        if high_issues:
            recommendations.append("⚠️ 주의 필요: 데이터 품질 개선이 필요합니다. 데이터 파이프라인 검토를 권장합니다.")
        ...
        if not recommendations:
            recommendations.append("✅ 데이터 품질이 양호합니다. 현재 상태를 유지하세요.")
        
        return recommendations
    
    def _generate_summary(self, overall_score: float, issues: List[QualityIssue]) -> str:
        """요약 생성"""
    ...
    async def _create_visualizations(self, report: Dict[str, Any]):
        ...
        try:
            ...
            fig = make_subplots(
                rows=2, cols=2,
                subplot_titles=('전체 품질 점수', '테이블별 품질', '이슈 심각도 분포', '품질 지표별 점수'),
            ...
            scores = [report['table_quality'][table].get('overall_score', 0) for table in tables]
            
            fig.add_trace(
                go.Bar(x=tables, y=scores, name="품질 점수"),
                row=1, col=2
            )
            ...
            # HTML 파일로 저장
            viz_path = self.reports_path / f"quality_visualization_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
