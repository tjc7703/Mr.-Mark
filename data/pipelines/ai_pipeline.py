...
class AIPipeline:
    ...
    def _define_models(self) -> Dict[str, AIModel]:
        ...
        return {
            'engagement_prediction': AIModel(
                ...
                features=['likes', 'comments', 'shares', 'hashtag_count', 'content_length', 'hour_of_day'],
            ...
            'trend_forecasting': AIModel(
                ...
                target='trend_score',
                hyperparameters={'changepoint_prior_scale': 0.05, 'seasonality_prior_scale': 10}
            ),
            'content_sentiment': AIModel(
                ...
                hyperparameters={'model_name': 'cardiffnlp/twitter-roberta-base-sentiment-latest'}
            ),
    ...
    async def _train_model(self, model_name: str, model_config: AIModel, data: pd.DataFrame):
        """개별 모델 학습"""
        try:
    ...
    async def _train_regression_model(self, model_name: str, model_config: AIModel, data: pd.DataFrame):
        """회귀 모델 학습"""
        try:
            # 특성과 타겟 분리
            ...
            # 모델 학습
            model = RandomForestRegressor(**model_config.hyperparameters, random_state=42)
            ...
            metrics = {
                ...
                'r2': r2,
                'mae': mae,
                'feature_importance': dict(zip(model_config.features, model.feature_importances_))
            }
    ...
    async def _train_time_series_model(self, model_name: str, model_config: AIModel, data: pd.DataFrame):
        """시계열 모델 학습 (Prophet)"""
    ...
    async def _train_nlp_model(self, model_name: str, model_config: AIModel, data: pd.DataFrame):
        ...
        try:
            # 감정 분석 파이프라인 생성
            model_name_hf = model_config.hyperparameters.get('model_name', 'cardiffnlp/twitter-roberta-base-sentiment-latest')
            
            sentiment_pipeline = pipeline(
                "sentiment-analysis",
                model=model_name_hf,
                tokenizer=model_name_hf
            )
    ...
    async def _train_clustering_model(self, model_name: str, model_config: AIModel, data: pd.DataFrame):
        """클러스터링 모델 학습"""
        try:
            # 특성 선택 및 전처리
            X = data[model_config.features].fillna(0)
            ...
            # 클러스터 분석 결과 저장
            cluster_analysis = data.groupby('cluster')[model_config.features].mean().to_dict()
            
            analysis_path = self.models_path / f"{model_name}_analysis.json"
            with open(analysis_path, 'w') as f:
                json.dump(cluster_analysis, f, indent=2)
            
            logger.info(f"클러스터링 모델 학습 완료: {model_name} - {n_clusters}개 클러스터")
            
        except Exception as e:
            logger.error(f"클러스터링 모델 학습 실패: {model_name} - {str(e)}")
    
    async def _train_classification_model(self, model_name: str, model_config: AIModel, data: pd.DataFrame):
        """분류 모델 학습"""
        try:
            # 특성과 타겟 분리
            X = data[model_config.features].fillna(0)
    ...
    async def _predict_engagement(self, model, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """참여율 예측"""
        try:
            # 입력 데이터 전처리
            features = ['likes', 'comments', 'shares', 'hashtag_count', 'content_length', 'hour_of_day']
    ...
    async def _predict_posting_time(self, model, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """최적 포스팅 시간 예측"""
        try:
            # 입력 데이터 전처리
            features = ['hour', 'day_of_week', 'content_type', 'hashtag_count']
    ...
    async def _predict_user_cluster(self, model, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """사용자 클러스터 예측"""
        try:
            # 입력 데이터 전처리
            features = ['post_count', 'avg_engagement_rate', 'followers', 'following']
